 ...
 */
public class ConSoMayManGold implements Runnable {

    public long second = 50;
    public long currlast = System.currentTimeMillis();
    public long rewardAmount = 90;
    public long cost = 1;
    public long min = 0;
    public long max = 100;
    ...
    private static ConSoMayManGold instance;
    private boolean inBettingPhase = true;

    private ConSoMayManGold() {
    }

    public static ConSoMayManGold gI() {
        if (instance == null) {
            instance = new ConSoMayManGold();
        }
        return instance;
    }
    ...
    public void newData(Player player, int point) {
        ...
        if (thoiVang == null || thoiVang.quantity < this.cost) {
            Service.gI().sendThongBao(player, "Bạn không đủ " + this.cost + " thỏi vàng để thực hiện");
            return;
        }
        ...
        data.point = point;
        data.conSoMayManNgoc = 0;
        data.conSoMayManVang = 1;
        players.add(data);
        InventoryService.gI().subQuantityItemsBag(player, thoiVang, (int) this.cost);
        InventoryService.gI().sendItemBags(player);
        Service.gI().sendThongBao(player, "Bạn đã chọn số " + point + " bằng " + this.cost + " thỏi vàng.");
    ...
    public void ramdom1SoLe(Player player) {
        ...
        do {
            ...
            if (players.stream().filter(d -> d.id == player.id).count() >= 50) { // Max 50 số lẻ
        ...
        InventoryService.gI().subQuantityItemsBag(player, thoiVang, (int) this.cost);
        InventoryService.gI().sendItemBags(player);
        Service.gI().sendThongBao(player, "Bạn đã chọn ngẫu nhiên số lẻ " + generatedPoint + " bằng " + this.cost + " thỏi vàng.");
        Service.gI().showYourNumber(player, strNumber((int) player.id), null, null, 0);
    }
    ...
    public void ramdom1SoChan(Player player) {
        ...
        InventoryService.gI().subQuantityItemsBag(player, thoiVang, (int) this.cost);
        InventoryService.gI().sendItemBags(player);
        Service.gI().sendThongBao(player, "Bạn đã chọn ngẫu nhiên số chẵn " + generatedPoint + " bằng " + this.cost + " thỏi vàng.");
        Service.gI().showYourNumber(player, strNumber((int) player.id), null, null, 0);
    }
        ...
        for (ConSoMayManData g : players) {
            if (id == g.id && g.point == result) {
                if (currentPlayer != null) {
                    if (g.conSoMayManVang == 1) {
                        finish = "Chúc mừng " + currentPlayer.name
                                + " đã thắng " + Util.numberToMoney(this.rewardAmount)
                                + " thỏi vàng với con số may mắn " + result;
                        Item item = ItemService.gI().createNewItem((short) 457, (int) this.rewardAmount);
                        InventoryService.gI().addItemBag(currentPlayer, item);
                        InventoryService.gI().sendItemBags(currentPlayer);
                    }
                } else {
                    if (g.conSoMayManVang == 1) {
                        finish = "Chúc mừng người chơi ID: " + g.id + " đã thắng " + Util.numberToMoney(this.rewardAmount) + " thỏi vàng với con số may mắn " + result;
                    }
                }
                break;
            }
    ...
    public void ResetGame(int result) {
        ...
        for (ConSoMayManData g : players) {
            Player player = Client.gI().getPlayer(g.id);
            if (player != null) {
                String msg = "Con số may mắn Vàng vòng này là: " + result;
